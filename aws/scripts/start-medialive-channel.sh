#!/usr/bin/env bash
# Start the MediaLive Channel using outputs from aws/out/medialive.outputs.json
set -euo pipefail
...
echo "Starting MediaLive Channel: $CHANNEL_ID (region=$REGION)"
aws medialive start-channel --region "$REGION" --channel-id "$CHANNEL_ID" >/dev/null

echo "Start requested. Current state:"
